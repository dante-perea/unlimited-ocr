"""NCBI integration service.

Discovery and retrieval of Open-Access papers from PubMed Central (PMC) using:
...
* A local on-disk cache (``settings.pdf_cache_dir_resolved``) for PDFs — the
  same cache the OCR pipeline (``app.routers.ocr``) reads from, so a fetched
  PDF can be OCRed by PMCID.

All upstream HTTP is async (``httpx``) and every E-utility call passes through a
rate limiter that respects NCBI's policy (3 requests/second without an API key,
10/second with one).
"""
...
def candidate_download_urls(url: str) -> list[str]:
    ...
    The PMC OA service historically returns ``ftp://`` links whose HTTPS mirror
    lives at ``/pub/pmc/<subpath>``. During PMC's 2026 dataset restructuring the
    files were relocated under a ``deprecated/`` prefix, so we additionally offer
    that path as a fallback. The first entry is always the canonical HTTPS
    mirror; subsequent entries are fallbacks tried on 404.
    """
...
class RateLimiter:
    ...
    Each :meth:`acquire` reserves a slot and ensures successive acquisitions are
    at least ``1/rate`` seconds apart. Because acquisitions are serialized through
    a lock, the configured ceiling is never exceeded.
    """
...
class NcbiService:
    ...
    A single :class:`httpx.AsyncClient` is owned by each instance so connection
    pooling is reused across requests. Use :func:`get_ncbi_service` for the
    process-wide singleton used by the router.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._limiter = RateLimiter(settings.ncbi_rate_per_second)
    ...
    def _with_default_params(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.settings.ncbi_has_api_key:
    ...
    async def _esearch_pmc(
        ...
        """Run ESearch against the PMC Open Access subset.

        Returns ``(id_list, total_count)``.
        """
    ...
    async def _fetch_abstracts(self, pmids: list[str]) -> dict[str, str]:
        """Fetch PubMed abstracts and return a ``{pmid: abstract_text}`` map.

        Failures are swallowed (returns an empty map) because abstracts are a
        best-effort enhancement of search results.
        """
        if not pmids:
            return {}
        try:
    ...
    def _doc_to_summary(
        ...
        return PaperSummary(
            ...
            authors=authors,
            journal=str(
                doc.get("fulljournalname") or doc.get("source") or ""
            ).strip(),
    ...
    async def search(
        ...
        if ids:
            ...
            # Collect pmids up front for a single batched abstract fetch.
            pmids: list[str] = []
            ...
            for uid in uids:
                ...
                for entry in doc.get("articleids", []) or []:
                    if (entry.get("idtype") or "").lower() in ("pubmed", "pmid"):
                        val = str(
                            entry.get("value") or entry.get("id") or ""
                        ).strip()
                        if val:
                            pmids.append(val)
                        break
    ...
    async def resolve_oa(
        ...
        """Query the PMC OA service.

        Returns a ``(record_attrs, download_links)`` tuple. Raises
        :class:`PaperNotFound` when the article is unknown to the OA service.
        """
    ...
    async def get_paper(self, pmcid: str) -> PaperDetail:
        ...
        summary = self._doc_to_summary(
            numeric_ids[0] if numeric_ids else "0", doc, {}
        )

        # Best-effort abstract snippet for this single paper.
    ...
    def _cache_path_for(self, pmcid: str) -> Path:
        """Where a paper's PDF is cached.

        Mirrors the OCR task's expectation: the cached PDF for ``PMC123`` lives
        at ``<pdf_cache_dir>/PMC123.pdf`` (see ``Settings.pdf_cache_dir_resolved``
        and ``OcrRunRequest.pmcid``).
        """
        return Path(self.settings.pdf_cache_dir_resolved) / f"{pmcid}.pdf"

    async def _download_pdf(self, url: str, dest: Path) -> int:
        """Stream-download a PDF to ``dest`` and return the byte count.

        Tries each :func:`candidate_download_urls` entry in order, so the
        relocation of PMC OA files under a ``deprecated/`` prefix is handled
        transparently.
        """
    ...
    async def _extract_pdf_from_tgz(self, url: str, dest: Path) -> int:
        ...
        try:
            ...
            raise NcbiError(
                f"Failed to read OA package: {exc}", status_code=502
            ) from exc
        dest.parent.mkdir(parents=True, exist_ok=True)
...
# --------------------------------------------------------------------------- #
# Process-wide singleton (used by the router via FastAPI dependency)
# --------------------------------------------------------------------------- #

_service_singleton: NcbiService | None = None


def get_ncbi_service() -> NcbiService:
    """Return the lazily-created, process-wide :class:`NcbiService`.

    A single shared ``httpx.AsyncClient`` is reused for connection pooling. The
    OCR router does not need this; it reads cached PDFs straight off disk.
    """
    global _service_singleton
    if _service_singleton is None:
        _service_singleton = NcbiService(get_settings())
    return _service_singleton


def reset_ncbi_service() -> None:
    """Drop the cached singleton (used by tests to inject a mock service)."""
    global _service_singleton
