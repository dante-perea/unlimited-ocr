...
from app.services.ocr_model import GpuRequirementError, _gpu_required_error, _looks_like_cuda_error
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="ocr_out_") as output_path:
        try:
            ...
            # On non-CUDA hosts the custom modeling code often fails at inference
            # (not just at load). Convert CUDA-flavored failures into a single,
            # clearly-worded GpuRequirementError so the frontend gets an actionable
            # message instead of a 500. (Real load-time failures are handled in
            # ocr_model._load_real_model.)
            if device != "cuda" and not is_mock and _looks_like_cuda_error(exc):
